# -*- coding: utf-8 -*-
"""
Created on Sun Apr 12 13:46:47 2020
...
f = fn_song_features('Nick Cave & The Bad Seeds - Straight To You-CYbOHXMtelU.wav')

df1 = pd.read_csv('Emotion_feat.csv',encoding='cp1252')
df2 = pd.read_csv('Emotion_feat1.csv',encoding='cp1252')

df1.dropna(inplace=True)
df2.dropna(inplace=True)

df_data = df1.append(df2)
print(df_data.describe().T)
df_data.drop('Unnamed: 0',inplace=True,axis=1)

ene = pd.Series(df_data.energy).reset_index()
val = pd.Series(df_data.valance).reset_index()

predictors = list(df_data.columns.drop(['valance','energy','song_name']))

x = df_data[predictors]

features_ene = ['contrast_var',
 'rolloff_mean',
 'contrast_std',
 'mfcc_delta_std',
 'poly_std',
...
rf_energy=RandomForestRegressor(random_state=2020,bootstrap=False,warm_start=True)
rf_energy.fit(x[features_ene],ene.energy)
print(rf_energy.predict(f[features_ene]))

rf_val=RandomForestRegressor(random_state=2020,bootstrap=False,warm_start=True)
rf_val.fit(x[fratures_val],val.valance)
print(rf_val.predict(f[fratures_val]))